...
export class ClaudeAuth {
  ...
  saveSession() {
    if (this.session) {
      ...
      localStorage.setItem('aura-claude-session', JSON.stringify(this.session));
    }
  }
  
  loadSession() {
    try {
      const saved = localStorage.getItem('aura-claude-session');
      if (saved) {
  ...
  clearSession() {
    localStorage.removeItem('aura-claude-session');
  }
